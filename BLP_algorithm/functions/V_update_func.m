...
function output=...
   ...
   EV=compute_EV_func(V_initial,weight_V);
    ...
    temp_1=sum(S_jt_data.*numer_1./denom_1,1);%1*ns*1*T
    
    numer_2=sum(reshape(weight,1,ns).*reshape(exp(v_i0t-V_initial),1,ns,1,T),2);%1*1*1*T
    temp_2=numer_2./reshape(S_0t_data,1,1,1,T);%1*1*1*T


    V_updated=log(exp(v_i0t)+temp_1.*(temp_2.^tune_param));%1*ns*1*T
