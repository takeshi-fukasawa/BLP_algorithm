...
%% Read inputs
x_elem=NaN(1,n_var,'gpuArray'); %Number of elements
for i=1:n_var
	x_size{1,i}=size(varargin{1,i});%%%
	x_elem(1,i)=prod(x_size{1,i},2);
end
x_0_mat=zeros(max(x_elem,[],2),n_var,'gpuArray');
x_max_mat=zeros(max(x_elem,[],2),n_var,'gpuArray');
x_min_mat=zeros(max(x_elem,[],2),n_var,'gpuArray');
...
DIST_table=zeros(ITER_MAX,n_var,'gpuArray');
...
fun_out_mat=zeros(max(x_elem),n_var,'gpuArray');
for i=1:n_var
	temp=fun_out_cell{i};
