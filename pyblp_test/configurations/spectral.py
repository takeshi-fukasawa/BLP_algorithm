...
from pyblp_test.configurations._linesearch import _nonmonotone_line_search_cruz, _nonmonotone_line_search_cheng
...
def _root_df_sane(func, x0, args=(), ftol=1e-8, fatol=1e-300, maxfev=1000,
    ...
    if line_search not in ('cheng', 'cruz','no'):
    ...
    while True:
        ...
        ## Second spec of Barzilai and Borwein (1988)
        sigma_k = np.vdot(s_k, s_k) / np.vdot(s_k, y_k)

        ## Specification proposed in Varadhan Roland (2008)
        sigma_k = np.sqrt(np.vdot(s_k, s_k) / np.vdot(y_k, y_k)) 
        
        #sigma_k=np.minimum(sigma_k,100.0)
